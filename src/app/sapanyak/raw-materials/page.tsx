    ...
    <div className="space-y-8 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        >
          <div className="space-y-1 min-w-0 flex-1">
            <span className="text-gray-400 dark:text-muted-foreground text-[10px] sm:text-xs font-bold uppercase tracking-wider block truncate">Nilai Aset Bahan Baku</span>
            <h3 className="text-xl sm:text-2xl font-extrabold text-gray-800 dark:text-foreground truncate">{formatRupiah(totalValuation)}</h3>
          </div>
        ...
        >
          <div className="space-y-1 min-w-0 flex-1">
            <span className="text-gray-400 dark:text-muted-foreground text-[10px] sm:text-xs font-bold uppercase tracking-wider block truncate">Jumlah Jenis Bahan</span>
            <h3 className="text-xl sm:text-2xl font-extrabold text-gray-800 dark:text-foreground truncate">{totalItemsCount} item</h3>
          </div>
          <span className="p-3 sm:p-3.5 rounded-2xl bg-pink-50 dark:bg-primary/10 text-secondary shrink-0">
        ...
        >
          <div className="space-y-1 min-w-0 flex-1">
            <span className="text-gray-400 dark:text-muted-foreground text-[10px] sm:text-xs font-bold uppercase tracking-wider block truncate">Perlu Restock (&lt; 5)</span>
            <h3 className={`text-xl sm:text-2xl font-extrabold truncate ${lowStockCount > 0 ? "text-amber-600 dark:text-amber-400 animate-pulse" : "text-gray-800 dark:text-foreground"}`}>
              {lowStockCount} item
            </h3>
          </div>
          <span className={`p-3 sm:p-3.5 rounded-2xl shrink-0 ${lowStockCount > 0 ? "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400" : "bg-gray-50 dark:bg-muted text-gray-400 dark:text-muted-foreground"}`}>
      ...
      <div className="bg-white dark:bg-card rounded-3xl border border-gray-200/60 dark:border-border shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-bold text-gray-800 dark:text-foreground">Inventaris Bahan Baku</h3>
            <p className="text-gray-400 dark:text-muted-foreground text-xs mt-0.5">Kelola data stok kertas wrap, pita, bunga, kardus, dll</p>
          </div>
          <button
              ...
              <tbody className="text-sm font-medium text-gray-700 dark:text-muted-foreground divide-y divide-gray-100 dark:divide-border">
                {filteredMaterials.length > 0 ? (
                  filteredMaterials.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-gray-50/20 dark:hover:bg-muted transition-colors">
                        <td className="px-6 py-4.5 font-bold text-gray-800 dark:text-foreground">{m.name}</td>
                        <td className="px-6 py-4.5">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-extrabold rounded-lg border ${
                            isLow
                              ? "bg-rose-50 dark:bg-rose-500/10 text-rose-650 dark:text-rose-400 border-rose-100 dark:border-rose-500/20 animate-pulse"
                              : "bg-gray-50 dark:bg-muted text-gray-750 border-gray-150 dark:border-border"
                          }`}>
                            {m.stock} {isLow && "(Hampir Habis)"}
                          </span>
                        </td>
                        <td className="px-6 py-4.5 text-gray-500 dark:text-muted-foreground capitalize">{m.unit}</td>
                        <td className="px-6 py-4.5 font-semibold text-gray-800 dark:text-foreground">{formatRupiah(m.price_per_unit)}</td>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Nama Bahan Baku</label>
                  <input
                    type="text"
                    required
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Stok Awal</label>
                    <input
                      type="number"
                      required
                      min="0"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Satuan / Unit</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Harga per Satuan</label>
                    <input
                      type="number"
                      required
                      min="0"
