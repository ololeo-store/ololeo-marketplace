      ...
      <div className="bg-white dark:bg-card rounded-3xl border border-gray-200/60 dark:border-border shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-800 dark:text-foreground">Keunggulan Toko (Store Values)</h3>
              <p className="text-gray-400 dark:text-muted-foreground text-xs mt-0.5">Kelola highlight kelebihan toko Anda yang ditampilkan di halaman depan</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredValues.map((val) => (
                  <motion.div
                    layout
                    key={val.id}
                    className="bg-white dark:bg-card rounded-[2rem] border border-gray-150 dark:border-border p-6 flex flex-col justify-between gap-5 relative group shadow-sm hover:shadow-md transition-all"
                  >
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="p-3.5 rounded-2xl bg-pink-50 dark:bg-primary/10 text-primary inline-block shrink-0">
                          {renderValueIcon(val.icon_name)}
                        </span>
                        <span className="flex items-center gap-1 text-[10px] font-bold text-gray-400 dark:text-muted-foreground bg-gray-50 dark:bg-muted border border-gray-150/50 dark:border-border px-2 py-0.5 rounded-lg shrink-0">
                          <ArrowUpDown className="w-3 h-3 text-gray-300 dark:text-muted-foreground" />
                          Urutan: {val.order_index}
                        </span>
                      </div>

                      <div className="space-y-1.5">
                        <h4 className="font-bold text-gray-800 dark:text-foreground text-sm leading-tight truncate">{val.title}</h4>
                        <p className="text-xs font-medium text-gray-500 dark:text-muted-foreground leading-relaxed min-h-[48px] line-clamp-3">
                          {val.content}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center justify-end gap-1.5 pt-3 border-t border-gray-100 dark:border-border shrink-0">
                      <button
                        onClick={() => handleOpenEdit(val)}
                        className="p-2 text-gray-450 dark:text-muted-foreground hover:text-primary hover:bg-pink-50/40 dark:hover:bg-primary/10 rounded-xl transition-all cursor-pointer"
                      >
                        <Edit className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDelete(val.id)}
                        className="p-2 text-gray-455 dark:text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50/40 dark:hover:bg-red-500/10 rounded-xl transition-all cursor-pointer"
                      >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Judul / Title</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Urutan Tampil</label>
                    <input
                      type="number"
                      required
                      min="0"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Pilih Icon Keunggulan</label>
                  <div className="grid grid-cols-6 gap-2 bg-gray-50/50 dark:bg-muted p-3.5 rounded-2xl border border-gray-150/50 dark:border-border max-h-[120px] overflow-y-auto scrollbar-thin">
                        ...
                        <button
                          ...
                          className={`p-2.5 rounded-xl border flex items-center justify-center cursor-pointer transition-all ${
                            isSelected
                              ...
                              : "bg-white dark:bg-card text-gray-400 dark:text-muted-foreground border-gray-150 dark:border-border hover:bg-gray-50/40 dark:hover:bg-muted hover:text-gray-600 dark:hover:text-muted-foreground"
                          }`}
                        >
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Konten / Deskripsi</label>
                  <textarea
                    rows={4}
                    required
        ...
        {isOverviewOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-card w-full max-w-4xl rounded-[2.5rem] border border-gray-100 dark:border-border shadow-2xl p-6 md:p-8 relative z-10 space-y-6 max-h-[90vh] overflow-y-auto scrollbar-thin"
            >
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-border pb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-muted-foreground mt-0.5">Berikut adalah simulasi grid "Kenapa Memilih Kami" pada Landing Page utama</p>
                </div>
                <button
                  onClick={() => setIsOverviewOpen(false)}
                  className="p-1.5 text-gray-400 dark:text-muted-foreground hover:text-gray-600 dark:hover:text-muted-foreground rounded-xl transition-colors cursor-pointer"
              ...
              <div className="border border-gray-150 dark:border-border rounded-2xl overflow-hidden bg-gray-50/50 dark:bg-muted shadow-inner">
                ...
                {/* Browser Page Body */}
                <div className="p-8 bg-gradient-to-b from-white dark:from-background to-gray-50/30 text-center space-y-12">
                  <div className="max-w-xl mx-auto space-y-2.5">
                    <span className="px-3.5 py-1 bg-pink-50 dark:bg-primary/10 text-primary text-[10px] font-extrabold tracking-wider uppercase rounded-full">
                      Kenapa Memilih Kami?
                    </span>
                    <h2 className="text-2xl font-extrabold text-gray-800 dark:text-foreground tracking-tight">
                      Komitmen Kami Untuk Memberikan Pelayanan Terbaik
                    </h2>
                    <p className="text-xs text-gray-450 dark:text-muted-foreground font-semibold leading-relaxed">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {values.length > 0 ? (
                      values.map((val) => (
                        <div
                          key={val.id}
                          className="bg-white dark:bg-card rounded-[2rem] border border-gray-100 dark:border-border p-6 flex flex-col items-center text-center gap-4 shadow-sm hover:shadow-md transition-all duration-300"
                        >
                          <span className="p-4 rounded-2xl bg-pink-50 dark:bg-primary/10 text-primary inline-flex shrink-0">
                            {renderValueIcon(val.icon_name)}
                          </span>
                          <div className="space-y-1.5">
                            <h4 className="font-extrabold text-gray-800 dark:text-foreground text-sm leading-tight">{val.title}</h4>
                            <p className="text-xs font-semibold text-gray-450 dark:text-muted-foreground leading-relaxed">
                              {val.content}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="col-span-3 py-10 text-center text-gray-400 dark:text-muted-foreground text-xs font-semibold">
