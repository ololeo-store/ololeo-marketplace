      ...
      <div className="bg-white dark:bg-card rounded-3xl border border-gray-200/60 dark:border-border shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-800 dark:text-foreground">Pengaturan Toko (Store Settings)</h3>
              <p className="text-gray-400 dark:text-muted-foreground text-xs mt-0.5">Konfigurasi variabel toko seperti kontak WhatsApp, alamat, jam buka, dan konfigurasi API gateway</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setIsOverviewOpen(true)}
              className="flex items-center justify-center gap-2 px-5 py-3.5 bg-gray-50 dark:bg-muted hover:bg-gray-100 dark:hover:bg-muted text-gray-755 dark:text-muted-foreground border border-gray-200 dark:border-border font-bold text-sm rounded-2xl transition-all cursor-pointer select-none"
            >
              <Eye className="w-4 h-4 text-gray-500 dark:text-muted-foreground" />
              ...
              <tbody className="text-sm font-medium text-gray-700 dark:text-muted-foreground divide-y divide-gray-100 dark:divide-border">
                {filteredSettings.length > 0 ? (
                  filteredSettings.map((s) => (
                    <tr key={s.key} className="hover:bg-gray-50/20 dark:hover:bg-muted transition-colors">
                      <td className="px-6 py-4 font-mono font-bold text-gray-800 dark:text-foreground text-xs shrink-0">
                        {s.key}
                      </td>
                      <td className="px-6 py-4 max-w-md font-semibold text-gray-650 dark:text-muted-foreground">
                        <div className="line-clamp-2 break-all whitespace-pre-wrap">{s.value}</div>
                      </td>
                      <td className="px-6 py-4 text-gray-400 dark:text-muted-foreground text-xs shrink-0">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                    <span>Nama Variabel (Key)</span>
                    <span title="Pilih variabel pengaturan toko dari daftar yang tersedia">
                      <HelpCircle className="w-3.5 h-3.5 text-gray-400 dark:text-muted-foreground cursor-help" />
                    </span>
                  </label>
                  {modalMode === "create" ? (
                    <div className="relative">
                      <button
                        ...
                        className="w-full px-4 py-3 bg-gray-50 dark:bg-muted border border-gray-200 dark:border-border focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-2xl outline-none text-xs font-bold text-gray-750 cursor-pointer shadow-sm text-left flex justify-between items-center custom-select"
                      >
                        <span>
                      ...
                      {isDropdownOpen && (
                        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-card border border-gray-150 dark:border-border rounded-2xl shadow-xl z-20 overflow-hidden py-1 max-h-[160px] overflow-y-auto scrollbar-thin animate-in fade-in slide-in-from-top-2">
                          ...
                          {PREDEFINED_SETTINGS.filter(
                            ...
                            <p className="text-center text-xs text-gray-400 dark:text-muted-foreground py-3 font-semibold">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-gray-100 dark:bg-muted border border-gray-200 dark:border-border rounded-2xl outline-none text-xs font-bold text-gray-500 dark:text-muted-foreground shadow-sm"
                    />
                  )}
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-455 dark:text-muted-foreground uppercase tracking-wider">Nilai Konfigurasi (Value)</label>
                  <textarea
                    rows={4}
                    required
        ...
        {isOverviewOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-card w-full max-w-lg rounded-[2.5rem] border border-gray-100 dark:border-border shadow-2xl p-6 md:p-8 relative z-10 space-y-6 max-h-[90vh] overflow-y-auto scrollbar-thin"
            >
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-border pb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-muted-foreground mt-0.5">Berikut adalah simulasi tampilan informasi toko Anda di website pembeli</p>
                </div>
                <button
                  onClick={() => setIsOverviewOpen(false)}
                  className="p-1.5 text-gray-400 dark:text-muted-foreground hover:text-gray-600 dark:hover:text-muted-foreground rounded-xl transition-colors cursor-pointer"
              ...
              <div className="border border-gray-150 dark:border-border rounded-2xl overflow-hidden bg-gray-50/50 dark:bg-muted shadow-inner">
                {/* Browser top-bar */}
                <div className="bg-gray-100 dark:bg-muted px-4 py-2 flex items-center gap-2 border-b border-gray-200 dark:border-border">
                  <div className="flex gap-1.5 shrink-0">
                    <span className="w-2.5 h-2.5 rounded-full bg-red-400"></span>
                    <span className="w-2.5 h-2.5 rounded-full bg-yellow-400"></span>
                    <span className="w-2.5 h-2.5 rounded-full bg-green-400"></span>
                  </div>
                  <div className="w-full mx-6 bg-white/80 dark:bg-card border border-gray-200 dark:border-border rounded-lg text-[10px] text-gray-450 dark:text-muted-foreground font-bold px-3 py-0.5 text-center truncate">
                    ...
                    <div className="flex items-center gap-2.5">
                      {settings.find((s) => s.key === "logo_url")?.value ? (
                        ...
                        <img
                          ...
                          alt="Logo Toko"
                          className="w-8 h-8 rounded-lg object-contain bg-gray-50 dark:bg-muted border border-gray-150 dark:border-border"
                          onError={(e) => {
                            (e.target as HTMLElement).style.display = "none";
                          }}
                        />
                      ) : (
                        <div className="w-8 h-8 rounded-lg bg-pink-50 dark:bg-primary/10 text-primary flex items-center justify-center font-bold text-sm">
                          {settings.find((s) => s.key === "shop_name")?.value?.[0]?.toUpperCase() || "S"}
                        </div>
                      )}
                      <span className="font-extrabold text-sm text-gray-800 dark:text-foreground">
                        {settings.find((s) => s.key === "shop_name")?.value || "Nama Toko Anda"}
                      </span>
                    </div>
                    <span className="text-[10px] font-bold text-primary px-3 py-1 bg-pink-50 dark:bg-primary/10 rounded-full">
                      Buka Sekarang
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex gap-2">
                        ...
                        <div>
                          <p className="text-[11px] font-extrabold text-gray-800 dark:text-foreground uppercase tracking-wider">Alamat Toko</p>
                          <p className="text-xs text-gray-500 dark:text-muted-foreground font-semibold leading-relaxed mt-0.5">
                      ...
                      {/* Map Graphic Preview */}
                      <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-2xl h-36 relative overflow-hidden flex items-center justify-center shadow-xs">
                        <div
                        ...
                        ></div>
                        <svg className="absolute inset-0 w-full h-full text-blue-200/50 dark:text-blue-300" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className="z-10 flex flex-col items-center gap-1 bg-white/95 dark:bg-card backdrop-blur-xs px-4 py-2.5 rounded-2xl border border-blue-100 dark:border-blue-500/20 shadow-md text-center max-w-[85%]">
                          <MapPin className="w-4.5 h-4.5 text-red-500 dark:text-red-400 animate-bounce" />
                          <p className="text-[9px] font-bold text-gray-800 dark:text-foreground line-clamp-1">
                            {settings.find((s) => s.key === "shop_address")?.value || "Lokasi Toko"}
                          </p>
                          <p className="text-[8px] text-gray-400 dark:text-muted-foreground font-semibold">
                    ...
                    <div className="flex flex-col justify-between gap-4">
                      <div className="space-y-4">
                        <div className="flex gap-2">
                          ...
                          <div>
                            <p className="text-[11px] font-extrabold text-gray-800 dark:text-foreground uppercase tracking-wider">Jam Operasional</p>
                            <p className="text-xs text-gray-500 dark:text-muted-foreground font-semibold mt-0.5">
                        ...
                        <div className="flex gap-2">
                          ...
                          <div>
                            <p className="text-[11px] font-extrabold text-gray-800 dark:text-foreground uppercase tracking-wider">Hubungi WhatsApp</p>
                            <p className="text-xs text-gray-500 dark:text-muted-foreground font-semibold mt-0.5">
              ...
              <div className="flex justify-end pt-4 border-t border-gray-100 dark:border-border">
                <button
                  type="button"
                  onClick={() => setIsOverviewOpen(false)}
                  className="px-6 py-3 bg-gray-100 dark:bg-muted hover:bg-gray-150 text-gray-700 dark:text-muted-foreground font-bold text-xs rounded-2xl transition-all cursor-pointer"
                >
                  Tutup Pratinjau
                </button>
              </div>
