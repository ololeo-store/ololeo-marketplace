...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-muted flex font-sans">
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-card border-r border-gray-200/80 dark:border-border flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:h-screen ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* Brand Header */}
        <div className="h-20 px-8 flex items-center justify-between border-b border-gray-100 dark:border-border shrink-0">
          ...
          <button
            onClick={() => setIsSidebarOpen(false)}
            className="p-1.5 text-gray-400 dark:text-muted-foreground hover:text-gray-600 dark:hover:text-muted-foreground rounded-lg lg:hidden"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Search Bar */}
        <div className="px-6 pt-4 pb-2 border-b border-gray-100/60 dark:border-border shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 bg-gray-50 dark:bg-muted border border-gray-200 dark:border-border focus:border-primary focus:ring-1 focus:ring-primary/20 rounded-xl outline-none text-xs transition-all font-semibold text-gray-600 dark:text-muted-foreground shadow-sm"
            />
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 py-6 overflow-y-auto space-y-6 scrollbar-thin">
          {filteredCategories.map((category) => (
            <div key={category.title} className="space-y-2">
              <h4 className="text-[10px] font-bold text-gray-400 dark:text-muted-foreground uppercase tracking-wider px-4">
                {category.title}
              </h4>
                    ...
                    <Link
                      ...
                      className={`flex items-center gap-3.5 px-4 py-3 rounded-2xl text-xs font-bold transition-all duration-300 ${
                        isActive
                          ...
                          : "text-gray-500 dark:text-muted-foreground hover:text-primary hover:bg-pink-50/30 dark:hover:bg-primary/10"
                      }`}
                    >
                      <Icon className={`w-4 h-4 ${isActive ? "text-white" : "text-gray-400 dark:text-muted-foreground"}`} />
                      <span>{item.name}</span>
                    </Link>
                  );
                })}
          ...
          {filteredCategories.length === 0 && (
            <p className="text-center text-xs text-gray-400 dark:text-muted-foreground pt-4 font-semibold">Menu tidak ditemukan</p>
          )}
        </nav>

        {/* Logout Footer Section */}
        <div className="p-4 border-t border-gray-100 dark:border-border bg-gray-50/50 dark:bg-muted shrink-0">
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-semibold text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors cursor-pointer"
          >
            <LogOut className="w-5 h-5 text-red-400 dark:text-red-400" />
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        {/* Header bar */}
        <header className="h-20 bg-white dark:bg-card border-b border-gray-200/80 dark:border-border px-6 md:px-8 flex items-center justify-between sticky top-0 z-30 shrink-0">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="p-2 text-gray-500 dark:text-muted-foreground hover:text-gray-700 dark:hover:text-muted-foreground bg-gray-50 dark:bg-muted hover:bg-gray-100 dark:hover:bg-muted rounded-xl transition-colors lg:hidden"
            >
              <Menu className="w-5 h-5" />
            </button>
            <h2 className="hidden md:block text-xl font-bold text-gray-800 dark:text-foreground">
              {getPageTitle()}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-gray-800 dark:text-foreground">{adminName}</p>
