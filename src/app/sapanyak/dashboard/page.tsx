...
export default function DashboardOverview() {
  ...
  const getPaymentBadge = (status: string) => {
    return status.toLowerCase() === "paid" ? (
      <span className="px-2.5 py-1 text-xs font-bold bg-green-50 text-green-600 border border-green-100 rounded-lg">
        Lunas
      </span>
    ) : (
      <span className="px-2.5 py-1 text-xs font-bold bg-gray-50 text-gray-500 border border-gray-150 rounded-lg">
        Belum Bayar
      </span>
    );
  };
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-100 text-red-600 rounded-3xl p-6 text-center font-semibold max-w-lg mx-auto">
  ...
  const statCards = [
    {
      ...
      icon: DollarSign,
      color: "from-green-400/20 to-emerald-400/20 text-emerald-600",
    },
    {
      title: "Total Orders",
      value: stats.totalOrders,
      icon: ShoppingBag,
      color: "from-blue-400/20 to-indigo-400/20 text-indigo-600",
    },
    {
      title: "Products Catalog",
      value: stats.totalProducts,
      icon: Award,
      color: "from-pink-400/20 to-rose-400/20 text-pink-600",
    },
    {
      title: "Categories",
      value: stats.totalCategories,
      icon: Layers,
      color: "from-purple-400/20 to-violet-400/20 text-violet-600",
    },
  ];
          ...
          return (
            ...
            >
              <div className="space-y-1.5 min-w-0 flex-1">
                <span className="text-gray-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider block truncate">
                  {card.title}
                </span>
                <h3 className="text-xl sm:text-2xl font-extrabold text-gray-800 truncate">{card.value}</h3>
              </div>
      ...
      >
        <div className="px-8 py-6 flex items-center justify-between border-b border-gray-100">
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">Pantau dan kelola pesanan masuk terakhir</p>
          </div>
          <Link
